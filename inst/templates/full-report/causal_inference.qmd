...
for(description in descriptions){
  ...
  for(cmInd in 1:nrow(cmIds)){
   cat(gt::as_raw_html(cohortMethodDiagnostics %>% 
                           dplyr::filter(.data$description == !!description) %>%
...
if(includeCohortMethod){
  ...
  cohortMethodResults <- cmResults
    # add Evidence synth if the results exist
  if(!is.null(cohortMethodES)){
    cohortMethodResults <- dplyr::bind_rows(cmResults, cohortMethodES)
  } 
...
for(description in descriptions){
  ...
  for(cmInd in 1:nrow(cmIds)){
    cat(gt::as_raw_html(cohortMethodResults %>% 
                           dplyr::filter(.data$description == !!description) %>%
                           dplyr::filter(.data$targetId == !!cmIds$targetId[cmInd]) %>%
                           dplyr::filter(.data$outcomeId == !!cmIds$outcomeId[cmInd]) %>% 
                           dplyr::select("databaseName","subsetTextTarget",
                                         "friendlyNameComp", "indicationName", 
...
### Active Comparator New User Design Plots
```{r}
#| label: cohort_method_plots
#| echo: false
#| results: 'asis'
#| fig-width: 13
#| out-width: "100%"
...
if(includeCohortMethod){
  ...
  analysisIds <- unique(cmResults$analysisId)
  
  for(analysisId in analysisIds){
  
    plots <- OhdsiReportGenerator::plotCmEstimates(
              cmData = cmResults, 
              cmDiagnostics = cohortMethodDiagnostics, 
              cmMeta = cohortMethodES, 
              includeCounts = TRUE, 
...
for(sccsInd in 1:nrow(sccsIds)){
  cat(gt::as_raw_html(sccsDiagnostics %>% 
...
sccsIds <- unique(sccsCombined[,c('targetId', 'outcomeId')])

for(sccsInd in 1:nrow(sccsIds)){
  cat(gt::as_raw_html(sccsCombined %>% 
                         dplyr::filter(.data$targetId == !!sccsIds$targetId[sccsInd]) %>%
                         dplyr::filter(.data$outcomeId == !!sccsIds$outcomeId[sccsInd]) %>%
                         dplyr::select("databaseName","subsetTextTarget", "description",
...
### Self-Controlled Case Series Design Plots
```{r}
#| label: sccs_plots
#| echo: false
#| results: 'asis'
...
if(includeSccs){
  ...
  analysisIds <- unique(sccsResults$analysisId)
  
  for(analysisId in analysisIds){
  
    plots <- OhdsiReportGenerator::plotSccsEstimates(
              sccsData = sccsResults, 
