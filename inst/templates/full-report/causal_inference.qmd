...
for(description in descriptions){
  ...
  outcomes <- unique(cohortMethodDiagnosticsTemp$outcomeId)
  
  for(outId in outcomes){
    outName <- cohortMethodDiagnosticsTemp$outcomeName[cohortMethodDiagnosticsTemp$outcomeId == outId][1]
    cat(paste0('\n\n##### Outcome: ',outName,'\n\n'))
    
    cohortMethodDiagnosticsTemp <- cohortMethodDiagnosticsTemp %>% 
  ...
  for(tciId in 1:nrow(tciIds)){
    ...
    boxName <- paste0(cmDiagnosticTable$friendlyNameTarget[1], ' vs ', cmDiagnosticTable$friendlyNameComp[1], 
                      ' for indication ', indicationNameTemp)
...
for(description in descriptions){
  ...
  outcomes <- unique(sccsDiagnosticsTemp$outcomeId)
  
  for(outId in outcomes){
    outName <- sccsDiagnosticsTemp$outcomeName[sccsDiagnosticsTemp$outcomeId == outId][1]
    cat(paste0('\n\n##### ',outName,'\n\n'))
    
    sccsDiagnosticsTemp <- sccsDiagnosticsTemp %>% 
    dplyr::filter(.data$outcomeId == !!outId)
  ...
  for(tiId in 1:nrow(tiIds)){
    ...
    boxName <- paste0(sccsDiagnosticTable$friendlyNameTarget[1],
                      ' for indication ', sccsDiagnosticTable$indicationName[1])
