...
if(length(comparatorIdsOfInterest) > 0 ){
  for(i in 1:length(comparatorIdsOfInterest)){
    if(!is.na(comparatorIdsOfInterest[i])){  
      name <- cohortDefinitions$friendlyName[cohortDefinitions$cohortDefinitionId == comparatorIdsOfInterest[i]]
      ...
      cat(
        CirceR::cohortPrintFriendly(cohortDefinitions$json[cohortDefinitions$cohortDefinitionId == comparatorIdsOfInterest[i]])
      )
